...
static inline void __obj_attach(cs_obj_t *o)
{
	...
	{
		fprintf(stderr, "cs: HEEEELP! Someone's trying"
...
void cs_obj_activate(cs_obj_t *o)
{
	if(o->flags | CS_OBJ_ACTIVE)
	{
		DBG(fprintf(stderr, "cs: Tried to activate active object!\n");
				)return;
	}
...
void cs_obj_deactivate(cs_obj_t *o)
{
	if(!(o->flags | CS_OBJ_ACTIVE))
	{
		DBG(fprintf(stderr, "cs: Tried to deactivate passive object!\n");
				)return;
	}
...
void cs_obj_free(cs_obj_t *o)
{
	...
	{
		DBG(fprintf(stderr, "cs: Tried to free free object %p!\n",
					o);
				)return;
	}
	cs_obj_deactivate(o);
...
{
	cs_obj_t *o = calloc(sizeof(cs_obj_t), 1);
	if(!o)
		return NULL;
...
{
	cs_engine_t *e = calloc(sizeof(cs_engine_t), 1);
	if(!e)
		return NULL;
