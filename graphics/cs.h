 ...
 *
 *	* Split each layer into two lists; on for
 ...
 * Scripting interface
 */
cs_obj_start_script(cs_obj_t *o, cs_script_t *s);
cs_obj_add_script(cs_obj_t *o, cs_script_t *s);

/*
 * Code generation
 *	Virtual machine:
 *		* Multithreaded core
 *		* Single data type: 32 bit integer
 *		* 32 bit + 32 bit args instruction sizes
 *		* Segmented memory; 16:16 pointers
 *		* Integrated atomic operations
 *		* Integrated thread safe FIFO queues
 *		* Global page for "registers"
 *		* Stack frames (pushes frame size)
 *		* Integrated "stack argument index manager"
 *		* "Interrupt" handling for external events
 *	Each thread has:
 *		* One LIFO stack
 *		* Local, private page for "registers"
 *		* Local registers reserved for PC, SP etc.
 */
cs_script_t *css_begin();
void css_frame(cs_script_t *b);		/* Wait for next frame */
void css_delay(cs_script_t *b, int n);	/* Delay for n frames */
void css_goto(cs_script_t *b, short seg, short n);	/* Far Jump */
void css_call(cs_script_t *b, short seg, short pos);	/* Far Call */
void css_pushi(cs_script_t *b, int n);	/* Push value n */
void css_push(cs_script_t *b, short seg, short pos);	/* Push from memory */
void css_pop(cs_script_t *b, short seg, short pos);	/* Pop to memory */
void css_end(cs_script_t *b);
#endif
 /*
 *
 * BUGS:
...
void cs_engine_advance(cs_engine_t *e, float to_frame);	/* 0 == reset timer*/
