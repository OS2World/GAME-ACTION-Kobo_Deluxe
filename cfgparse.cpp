...
int cfg_key_t::copy(cfg_key_t *from)
{
...
void cfg_key_t::write(FILE *f)
{
}


void cfg_key_t::set_default()
{
}


int cfg_key_t::is_your_var(void *var)
{
	return 0;
}
...
config_parser_t &config_parser_t::operator = (config_parser_t &from)
{
	...
	while(k)
	{
		...
		while(fk)
		{
			...
			{
				if(k->copy(fk) < 0)
					fprintf(stderr, "config_parser_t: Tried"
...
void config_parser_t::desc(const char *text)
{
	if(!last_key)
	{
		fprintf(stderr, "config_parser_t::description() called"
	...
	while(i < argc)
	{
		...
		while(k)
		{
			if(k->test(argv[i]))
			{
				...
				{
					fprintf(stderr, "ERROR: Too few parameters to"
							" argument '%s'!\n",
							k->name);
					return -2;
				}
