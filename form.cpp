...
void kobo_form_t::item(const char *cap, int value, int ind)
{
	if(current_list)
	{
		...
		fprintf(stderr, "kobo_form_t::form_item(): No list!\n");
}
...
void kobo_form_t::editor(int handle, int tag)
{
	...
	{
	  default:
		fprintf(stderr, "kobo_form_t: Unknown data type!\n");
		break;
	  case CFG_BOOL:
	  ...
	  case CFG_FLOAT:
		...
		break;
	  case CFG_STRING:
		fprintf(stderr, "kobo_form_t: String editor not yet implemented!\n");
		break;
	}
}
...
void kobo_form_t::build_all()
{
	...
	while(w)
	{
		if(w->user)
		{
			...
			{
				w->value(*val);
				DBG(printf("Initialized to %d from prefs\n",
						w->value());)
			}
			else if(_data)
			{
				w->value(_data->get_i(w->user2));
				DBG(printf("Initialized to %d from config_parser\n",
						w->value());)
			}
		}
		w = w->next;
